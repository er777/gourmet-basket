...
class DataGrid {
    ...
    */
    ...
    var $RequestFile = "";
    var $JsRequestFile = "";
    function DataGrid($data = '', $RequestFile = '', $JsRequestFile = '') {
        ...
        if ($RequestFile)
            ...
            $this->RequestFile = $_SERVER['PHP_SELF'];
        
        if ($JsRequestFile)
            $this->JsRequestFile = $JsRequestFile;
        else
            $this->JSRequestFile = '';
    }
    ...
    var $columns = array();
    function AddColumnReadonly($colname, $coltitle, $attr = false) {
        $this->columns[] = array(
            'colname' => $colname,
            'coltitle' => $coltitle,
            'coltype' => 'readonly',
    ...
    function AddColumnHidden($colname = '', $coltitle = '',$attr = false) {
        $this->columns[] = array(
            'colname' => $colname,
            'coltitle' => $coltitle,
            'coltype' => 'hidden',
    ...
    function AddColumnText($colname, $coltitle, $attr = false) {
        $this->columns[] = array(
            'colname' => $colname,
            'coltitle' => $coltitle,
            'coltype' => 'text',
    ...
    function AddColumnCheckbox($colname, $coltitle, $attr = false) {
        $this->columns[] = array(
            'colname' => $colname,
            'coltitle' => $coltitle,
            'coltype' => 'checkbox',
    ...
    function AddColumnSelect($colname, $coltitle, $attr = false, $selectvalues) {
    ...
    function AddColumnSelectkey($colname, $coltitle, $attr = false, $selectvalues) {
        if (!is_array($selectvalues))
    ...
    function AddColumnTextarea($colname, $coltitle, $attr = false) {
    ...
    function AddColumnDelete($colname, $coltitle, $attr = false) {
        $this->columns[] = array(
            'colname' => $colname,
            'coltitle' => $coltitle,
            'coltype' => 'delete',
    ...
    function attributes(&$val) {
        return is_array($val) ? implode(array_map(create_function('$key, $value', 'return $key.\'="\'.$value.\'"\' ;'), array_keys($val), array_values($val))) : $val;
        //return $value;
    }
    ...
    */
    ...
    function GenerateTable() {
        ...
        //include JS
        return $this->GenerateJS($this->JsRequestFile) . "<span id=\"$JsClass.span\">\n" . $s . '</span>';
    }
    ...
    function GenerateNav() {
        ...
        $table .= " <span id=\"$JsClass.navbusy\" style=\"background-color:yellow;display: none;\"></span>";
        $table .= "<br>\n";
        return $table;
    }
    ...
    function GenerateTableBody() {
        ...
        foreach ($this->columns as $k => $v) {
            if ($v['coltype'] == 'delete')
                $s .= "\t<th>$v[coltitle]</th>\n";
            elseif ($v['coltype'] != 'hidden')
                $s .= "\t<th onclick=\"dg_onHeadClick($this->JsClass,'$v[colname]')\">$v[coltitle]</th>\n";
            $c++;
        }
        while ($c < $colcnt) {
            $s .= "\t<th>&nbsp;</th>\n";
        ...
        foreach ($this->data as $k => $rowdat) {
            ...
            foreach ($rowdat as $kk => $coldat) {
                ...
                switch ($col['coltype']) {
                    ...
                    case 'delete':
                        $s .= "\n\t<td>"
                           .  '<a class="button" onclick="del( \'#'.$curRow.'\', '.$rowid.' )">Delete</a>';
                        $s .= "\n\t</td>";
                        break;
                    case 'checkbox':
                        $s .= "\n\t<td><input type=\"checkbox\" id=\"$HtmlId\" onClick=\"dg_checkChange($JsClass,'$rowid','$colid','$HtmlId')\"" 
                           . ($coldat ? 'checked' : '') . '>';
                        $s .= "\n\t</td>";
                        break;
                    case 'selectkey':
                        $s .= "\n\t<td id=\"$HtmlId\" onClick=\"dg_editCell($JsClass,'$rowid','$colid','$HtmlId')\"><input type=\"hidden\" id=\"$HtmlId.h\" value=\"" .
                            htmlspecialchars($coldat, ENT_QUOTES) . "\">";
    ...
    function GenerateJS($path) {
        $JsClass = $this->JsClass;
        $js = '';
        //determine if we need the 'includes'
        global $DataGrid_JsIncluded;
        if (!$DataGrid_JsIncluded) {
            $js .= '<script type="text/javascript" src="'.$path.'sack.js"></script>';
            $js .= '<script type="text/javascript" src="'.$path.'datagrid.js"></script>';
        ...
        if (is_array($this->columns))
            foreach ($this->columns as $k => $v) {
                $js .= "$JsClass.m_columns['$v[colname]']={'coltype':'$v[coltype]','attributes':'$v[attributes]'};\n";
                $opt = array();
                if (isset($v['selectvalues']) && is_array($v['selectvalues']))
                    foreach ($v['selectvalues'] as $kk => $vv) {
                        $opt[] = "'" . htmlentities($kk, ENT_QUOTES) . "':'" . htmlentities($vv,
                            ENT_QUOTES) . "'";
                    }
                if ($opt)
